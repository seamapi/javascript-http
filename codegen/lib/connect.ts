...
import { getResourceLayoutContexts } from './layouts/resources.js'
import {
...
export const connect = (
  ...
  for (const resource of getResourceLayoutContexts(blueprint)) {
    files[`${resourcesPath}/${resource.fileName}`] = {
      contents: Buffer.from(
        `/* Automatically generated by codegen/smith.ts. */\n\nexport type ${resource.typeName} = ${resource.type}\n`,
      ),
    }
    resourceExports.push(
      `export type { ${resource.typeName} } from './${resource.fileName.replace(/\.ts$/, '.js')}'`,
    )
  }
  files[`${resourcesPath}/index.ts`] = {
    contents: Buffer.from(`${resourceExports.join('\n')}\n`),
  }
